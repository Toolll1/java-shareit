...
public class BookingService {
    ...
    public List<BookingDto> getUsersBooking(Integer userId, String state) {
        ...
        switch (state) {
            ...
            case "REJECTED":
                bookings = bookingRepository.
                        findAllByBookerIdAndStatusInOrderByStartDesc(userId, rejected);
                break;
            default:
    ...
    public List<BookingDto> getBookingsForUsersItems(Integer userId, String state) {
        ...
        if (state.equals("ALL")) {
            ...
            bookings = bookingRepository.
                    findAllByOwnerIdAndStatusOrderByStartDesc(userId, BookingStatus.WAITING);
        } else if (state.equals("REJECTED")) {
            bookings = bookingRepository.
                    findAllByOwnerIdAndStatusInOrderByStartDesc(userId, rejected);
        } else {
            throw new BadRequestException
                    ("Unknown state: " + state);
        }
